...
def get_links(html):
    ...
    temp = tree.xpath('//div[@class = "detailWeather floatL"]/div[@class = "temp"]/span[@ class = "temperature_value"]/text()')[0]
    weatther_1 = tree.xpath('//div[@class = "detailWeather floatL"]/div[@class = "someTemp"]/span[@ class = "tempStyle"]/text()')[0]
    feels_like = tree.xpath('//div[@class = "detailWeather floatL"]/div[position() = 2]/@title')[0]
    return temp, weatther_1, feels_like
...
def link_crawler(start_url, link_regex, robots_url=None, user_agent='wswp',
    ...
    while crawl_queue:
        ...
        if no_robots or rp.can_fetch(user_agent, url):
            ...
            #    datas.append(link)
            #return datas
            result = 'The temperature is: ' + get_links(html)[0] + 'C. Feels like ' + get_links(html)[1] + ". " + get_links(html)[2]
            return result
...
def weather(entities, entity, values):
    try:
        if 'today' in values:
            site = 'https://www.meteoprog.ua/en/weather/' + values[entities.index('city')]
        elif 'tomorrow' in values:
            site = 'https://www.meteoprog.ua/en/weather/' + values[entities.index('city')] + '/?showtomorrow'
        else:
            site = 'https://www.meteoprog.ua/en/weather/' + values[entities.index('city')]
